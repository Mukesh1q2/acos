...
import { Brain, ShieldCheck, Lightbulb, Zap, AlertCircle, Cpu, Network } from "lucide-react";
...
const keyInnovations = [
  ...
  },
  {
    ...
    description: "Stiefel Manifold St(d,K) parameterization ensures SᵀS = Iₖ, giving mathematically proven zero inter-thread interference: ⟨Sᵢ, Sⱼ⟩ = 0 for all i ≠ j",
    color: "teal",
  },
  {
    icon: <Brain className="w-5 h-5" />,
    title: "Three-Tier Memory",
    description: "Working Memory (Stiefel matrix Sₜ), Episodic Memory (HNSW vector store), Semantic Memory (Knowledge Graph). Retrieval in O(d·logM).",
    color: "green",
  },
  {
...
const v2Corrections = [
  ...
  { num: 7, correction: "ε term identified as structural, not noise-based" },
];
      ...
      <div className="relative z-10 px-6 py-16 md:py-24 max-w-5xl mx-auto">
        ...
        >
          {[
            ...
            {
              icon: <ThreadDotIcon className="w-6 h-6" />,
            ...
            <motion.div
              ...
              className={`relative p-6 rounded-xl bg-gradient-to-br ${item.gradient} border ${item.border} backdrop-blur-sm group hover:scale-[1.02] transition-transform duration-300`}
            >
          ...
          <div className="max-w-lg mx-auto space-y-2">
            {[
              ...
              <motion.div
                ...
                className={`${layer.color} ${layer.textColor} text-center py-3 px-4 rounded-lg text-sm font-medium`}
              >
                {layer.name}
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {keyInnovations.map((inn, i) => {
              ...
              return (
                ...
                >
                  <Card className={`border-border/30 h-full hover:${colors.border} transition-colors duration-300`}>
...
function ThreadDotIcon({ className }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
